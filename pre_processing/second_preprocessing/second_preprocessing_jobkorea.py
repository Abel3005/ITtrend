...
async def main():
    ...
    while last_evaluated_key:
        ...
        last_evaluated_key = response.get('LastEvaluatedKey', None)
        time.sleep(5)
    precesed_data= set(map(lambda x: str(x['pid']), _data))
    with open("./jobkorea.json") as f:
        raw_data = f.readlines()
    data = [json.loads(row) for row in raw_data]
    _all = jobkorea(logger).pre_processing_first(data)
    _filter = _all[_all['id'].apply(lambda x : str(x) not in precesed_data)]    
    unique_df = _filter.drop_duplicates(subset='id', keep='first').to_dict('records')
    
    batch_count = (len(unique_df) // batch_size) +1
    start_idx = 0
    for b in range(5):
        final_idx = min(len(unique_df),start_idx+batch_size)
