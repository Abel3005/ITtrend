...
class jobkorea:
    ...
    def get_job(self, query='duty=1000229%2C1000230%2C1000231%2C1000232%2C1000233%2C1000234%2C1000235%2C1000236%2C1000237%2C1000238%2C1000239%2C1000240%2C1000241%2C1000242%2C1000243%2C1000244%2C1000245%2C1000246&sort=6',flag='daily'):
        if flag == 'all':
            ...
            response.close()
            del soup
        print(f"[jobkorea] get job list {flag}")
        if flag == 'daily':
            all_num = 100
        print(f"[jobkorea] get job list {all_num}")
    
        #log(f"query({query} include {all_num} jobs)",4)
        pagenum = (all_num //40) + 1
        for p in range(1,pagenum+1):
            target_url = f'https://m.jobkorea.co.kr/Recruit/JobList/arealist?{query}&page={p}&sort=6'
            try:
                ...
                print(f"[jobkorea] get job list{target_url}:{e}")
        ...
        for job_id in self.all_dict:
            try:
                ...
                print(f"[jobkorea] {base_url + job_id} error ")
...
def main():
    try:
        ...
        with open("./API_KEYS.json", "r") as f:
            key = json.load(f)
        df.to_json("./jobkorea.json",orient='records',lines=True,force_ascii=False,date_format='iso')
        print(len(instance.all_dict))
    except Exception as e:
        print(e)
    else:
